...
Vue.component('qualityLog', {
  ...
  mounted: function mounted() {
    ...
    linkTag.setAttribute('href', "css/qualityLog.css");
    linkTag.setAttribute('rel', "stylesheet");
    document.body.appendChild(linkTag);

    var url = baseURL2 + "/api/quality/qualityLogs/listMonth";
    axios.post(url, {
      ...
      self.noData = false;
      console.log('qualityLog res is: ' + JSON.stringify(response));
      self.logs.length = 0;
      if (!isEmpty(response.data.result.list)) {
        ...
        response.data.result.list.forEach(function (item, index) {
          tempObj.logName = item.qualitySituation;
