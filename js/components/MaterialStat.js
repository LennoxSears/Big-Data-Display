...
Vue.component('materialstat', {
  ...
  mounted: function mounted() {
    ...
    var url = baseURL + "/api/stuff/stat/stuffStatisticsPutOut"; // var url = "http://10.2.100.240:8060/api/stuff/stat/stuffStatisticsPutOut?projId=7784644783492177920"+"&month="+monthStr;
    ...
    axios.get(url, {
      ...
      //console.log("materialStat res is: " + JSON.stringify(response));
      var materialNames = [];
      var inLibs = [];
      var outLibs = [];
      if (!isEmpty(response.data.result)) {
        ...
        response.data.result.forEach(function (item, index) {
          materialNames.push(item.stuffName);
          inLibs.push(item.putCount);
          outLibs.push(item.outCount);
        });
  ...
  methods: {
    initEcharts: function initEcharts(materialNames, inLibs, outLibs, height) {
      var self = this;
      var echartDom = document.getElementById('materialEcharts');
      ...
      var echartDom2 = document.getElementById('materialStatScroll');
